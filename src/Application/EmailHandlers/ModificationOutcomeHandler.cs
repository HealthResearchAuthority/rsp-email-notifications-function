...
/// <summary>
/// Handles the MOD_OUTCOME (Modification Outcome) email event.
/// 
/// This handler is responsible for:
/// 1. Retrieving user email addresses from User Management
/// 2. Extracting modification outcome data from the envelope
...
/// </summary>
public class ModificationOutcomeHandler(
    ...
    : IEmailHandler
{
    ...
    /// </summary>
    public string EventType => "MOD_OUTCOME";

    /// <summary>
    /// Handles the Modification Outcome notification workflow.
    /// 
    /// Flow:
    /// - Fetch users by IDs from the envelope
    /// - Deserialize template-specific data (modification outcome details)
    ...
    public async Task Handle(EmailEnvelope envelope)
    {
        ...
        // ------------------------------------------------------------
        var additionalData = envelope.Data.Deserialize<ModificationOutcomeDto>();
        ...
        foreach (var message in uniqueEmails.Select(email => new EmailNotificationMessage
                 {
                     EmailTemplateId = envelope.EmailTemplateId, // Template tied to MOD_OUTCOME
                     EventType = envelope.EventType,             // Should be "MOD_OUTCOME"
