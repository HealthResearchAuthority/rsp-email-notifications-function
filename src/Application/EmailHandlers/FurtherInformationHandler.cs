...
/// <summary>
/// Handles the FURTHER_INFO (Further Information Requested) email event.
/// 
/// This handler is responsible for:
/// 1. Retrieving user email addresses from User Management
/// 2. Extracting further information request data from the envelope
...
/// </summary>
public class FurtherInformationHandler(
    ...
    : IEmailHandler
{
    ...
    /// </summary>
    public string EventType => "FURTHER_INFO";

    /// <summary>
    /// Handles the Further Information notification workflow.
    /// 
    /// Flow:
    /// - Fetch users by IDs from the envelope
    /// - Deserialize template-specific data (further information request details)
    ...
    public async Task Handle(EmailEnvelope envelope)
    {
        ...
        foreach (var message in uniqueEmails.Select(email => new EmailNotificationMessage
                 {
                     EmailTemplateId = envelope.EmailTemplateId, // Template tied to FURTHER_INFO
                     EventType = envelope.EventType,             // Should be "FURTHER_INFO"
